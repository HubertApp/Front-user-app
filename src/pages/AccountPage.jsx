...
function SettingRow({ icon, label, sub, trailing, onClick, switchRole = false, checked }) {
  const Tag = onClick ? 'button' : 'div';
  return (
    <Tag
      type={onClick ? 'button' : undefined}
      role={switchRole ? 'switch' : undefined}
      aria-checked={switchRole ? checked : undefined}
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-3.5 py-3.5 text-left ${onClick ? 'pressable' : ''}`}
    >
    ...
    <div className={`min-h-screen bg-warm-bg text-ink pb-28 md:pb-12 ${collapsed ? 'md:pl-16' : 'md:pl-64'}`}>
      <main id="main-content" className="max-w-md mx-auto px-5 md:px-8">
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          {USER_STATS.map(s => (
            <div key={s.lbl} className="bg-white rounded-2xl border border-line p-3 text-center">
        ...
        <SettingsSection title="Coordonnées personnelles">
          <SettingRow icon="fa-user" label="Informations personnelles" sub="Nom, prénom, date de naissance"
            trailing={<i className="fa-solid fa-chevron-right text-soft text-[11px]" />} />
        ...
        <SettingsSection title="Préférences">
          <SettingRow icon="fa-bell" label="Notifications" sub="Alertes trafic, rappels de départ"
            trailing={<span className={`togg ${notifs ? '' : 'off'}`} aria-hidden="true" />}
            switchRole checked={notifs} onClick={() => setNotifs(v => !v)} />
          <SettingRow icon="fa-moon" label="Mode sombre" sub="Thème nuit activé globalement"
            trailing={<span className={`togg ${dark ? '' : 'off'}`} aria-hidden="true" />}
            switchRole checked={dark} onClick={toggleDark} />
